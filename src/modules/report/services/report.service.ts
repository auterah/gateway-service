...
export class ReportService {
  ...
  fetchStatisticsByAppId(appId: string, findOpts: FindDataRequestDto) {
    try {
      const isDateRange = findOpts.start_date && findOpts.end_date;
      const startDate =
        isDateRange && DateUtils.parseHyphenatedDate(findOpts.start_date);
      ...
      return this.txnRepo.fetchStatistics(opts);
    } catch (e) {
      throw new HttpException(
        e?.message || 'Something went wrong',
        e?.status || HttpStatus.EXPECTATION_FAILED,
  ...
  fetchOverviewForAll(customerId: string, findOpts: FindDataRequestDto) {
    try {
      const isDateRange = findOpts.start_date && findOpts.end_date;
      const startDate =
        isDateRange && DateUtils.parseHyphenatedDate(findOpts.start_date);
      const endDate =
        isDateRange && DateUtils.parseHyphenatedDate(findOpts.end_date);
      const opts: FindManyOptions<MailTransaction> = { where: { customerId } };
      const isInvalidDates = startDate || endDate;
